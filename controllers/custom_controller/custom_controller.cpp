...
CCustomController::CCustomController() :
   m_pcWheels(NULL),
   m_pcProximity(NULL),
...
void CCustomController::Init(TConfigurationNode& t_node) {
   /*
...
void CCustomController::ControlStep() {
   ...
   if(fMaxReadVal > 0.0f) {
     ...
     if(unMaxReadIdx == 0 || unMaxReadIdx == 1) {
         //std::cout << "Value of fMaxReadVal is " << fMaxReadVal;
 ...
 */
REGISTER_CONTROLLER(CCustomController, "custom_controller")
