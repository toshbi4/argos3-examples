/**
 * @file <argos3/plugins/simulator/sensors/myepuck_proximity_default_sensor.cpp>
 *
...
#include "myepuck_proximity_default_sensor.h"
...
namespace argos {
   ...
   CMyEPuckProximityDefaultSensor::CMyEPuckProximityDefaultSensor() :
   ...
   void CMyEPuckProximityDefaultSensor::SetRobot(CComposableEntity& c_entity) {
      try {
   ...
   void CMyEPuckProximityDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_MyEPuckProximitySensor::Init(t_tree);
         /* Show rays? */
   ...
   void CMyEPuckProximityDefaultSensor::Update()
   {
   ...
   void CMyEPuckProximityDefaultSensor::Reset()
   {
   ...
   Real CMyEPuckProximityDefaultSensor::CalculateReading(Real f_distance)
   {
      //return Exp(-f_distance);

      // from the my_e-puck model by Lorenzo Garattoni and Gianpiero Francesca
      Real value = 0.0f;
   ...
   REGISTER_SENSOR(CMyEPuckProximityDefaultSensor,
                   "myepuck_proximity", "default",
                   "Danesh Tarapore [daneshtarapore@gmail.com]",
                   "1.0",
                   "The E-Puck proximity sensor.",

                   "This sensor accesses the myepuck proximity sensor. For a complete description\n"
                   "of its usage, refer to the ci_myepuck_proximity_sensor.h interface. For the XML\n"
