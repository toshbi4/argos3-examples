/**
 * @file <argos3/plugins/robots/generic/control_interface/ci_myepuck_proximity_sensor.cpp>
 *
 * @author Danesh Tarapore <daneshtarapore@gmail.com>
 */

#include "ci_myepuck_proximity_sensor.h"
...
namespace argos 
{
   ...
   CCI_MyEPuckProximitySensor::CCI_MyEPuckProximitySensor() :
      m_tReadings(8)
   {
...
#ifdef ARGOS_WITH_LUA
   void CCI_MyEPuckProximitySensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_MyEPuckProximitySensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_MyEPuckProximitySensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_MyEPuckProximitySensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
